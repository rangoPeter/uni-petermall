<template>
	<view class="detailContainer">
		<view class="header">
			<icon class="iconfont icon-shouye2" @click="toHome"></icon>
			<text>网易严选 </text>
			<view class="shopCart" @click="toCart">
				<icon class="iconfont icon-gouwuche2"></icon>
				<text class="count">{{goodsList.length}}</text>
			</view>
		</view>
		
		<!-- 内容区 -->
		<scroll-view class="content" scroll-y="true">
			<image class="detailImg" :src="goodsDetail.listPicUrl" mode=""></image>
			<view class="tag">{{goodsDetail.promTag}}</view>
			<text class="price">￥ {{goodsDetail.retailPrice}}</text>
			<view class="info">{{goodsDetail.name}}</view>
...
<script>
	...
	export default {
		...
		async mounted() {
			const {goodId: id} = this.$mp.query
			const res = await request('/getGoodsById', {id: parseInt(id)})
